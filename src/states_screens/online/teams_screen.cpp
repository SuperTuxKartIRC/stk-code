...
NetworkTeamsSetupScreen::NetworkTeamsSetupScreen() : Screen("teams_setup.stkgui")
{
}

// -----------------------------------------------------------------------------

void NetworkTeamsSetupScreen::loadedFromFile()
{
}

// ----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::eventCallback(Widget* widget, const std::string& name,
    const int playerID)
{
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::beforeAddingWidget()
{
    ...
    int nb_players = NetworkConfig::get()->getNetworkPlayers().size();
    for (int i = 0; i < nb_players; i++)
    {
        RemoteKartInfo& rki = RaceManager::get()->getKartInfo(i);
        const std::string& kart_name = rki.getKartName();

        bool has_always_on_spectators = false;
        auto players = STKHost::get()
            ->getPlayersForNewGame(&has_always_on_spectators);

        //players[i]->getKartName()

        //NetworkString kart(PROTOCOL_LOBBY_ROOM);
        //kart.encodeString(m_kart_widgets[n].m_kart_internal_name);

        const KartProperties* props = kart_properties_manager->getKart("tux"); // kart_name // TODO : Dois être capable de trouver le nom du personnage choisit 
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::init()
{
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::tearDown()
{
    Widget* central_div = getWidget<Widget>("central_div");
...
// -----------------------------------------------------------------------------
GUIEngine::EventPropagation NetworkTeamsSetupScreen::filterActions(PlayerAction action,
    int deviceID,
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::onUpdate(float delta)
{
    int nb_players = (int)m_kart_view_info.size();
...
// ----------------------------------------------------------------------------
bool NetworkTeamsSetupScreen::areAllKartsConfirmed() const
{
...
// -----------------------------------------------------------------------------
int NetworkTeamsSetupScreen::getNumConfirmedKarts()
{
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::updateKartViewsLayout()
{
    Widget* central_div = getWidget<Widget>("central_div");
    ...
    const int center_x = central_div->m_x + central_div->m_w / 4; // 2
    const int center_y = central_div->m_y + central_div->m_h / 4; // 2
    ...
    const int center_x_per_team[4] = { (central_div->m_x + center_x) / 2,
                                       (central_div->m_x + central_div->m_w / 2 + center_x) / 2,
                                       (central_div->m_x + central_div->m_w + center_x) / 2,
                                       (central_div->m_x + center_x) * 3 }; // Pas placer correctement 
    ...
    for (int i = 0; i < nb_players; i++)
    {
        ...
        const int pos_y = start_y[teams] + cur_row * kart_view_size;
        ...
        const int pos_x = center_x_per_team[teams] + cur_col * kart_view_size - nb_karts_in_this_row * kart_view_size / 2; // 2
...
// -----------------------------------------------------------------------------
bool NetworkTeamsSetupScreen::onEscapePressed()
{
...
// -----------------------------------------------------------------------------
void NetworkTeamsSetupScreen::prepareGame()
{
