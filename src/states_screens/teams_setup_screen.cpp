    ...
    const int playerID)
{
    ...
    else if (name == "team_1")
    {
        if (m_kart_view_info.size() == 1)
            changeTeam(0, KART_TEAM_1, m_team1Color);
    }
    else if (name == "team_2")
    {
        if (m_kart_view_info.size() == 1)
            changeTeam(0, KART_TEAM_2, m_team2Color);
    }
    else if (name == "team_3")
    {
        if (m_kart_view_info.size() == 1)
            changeTeam(0, KART_TEAM_3, m_team3Color);
    }
    else if (name == "team_4")
    {
        if (m_kart_view_info.size() == 1)
            changeTeam(0, KART_TEAM_4, m_team4Color);
    }
}   // eventCallback
...
void TeamsSetupScreen::changeTeam(int player_id, KartTeams teams, KartTeamsColor teamColor)
{
    ...
    if (m_kart_view_info.size() == 1)
    {
        ...
        World::getWorld()->setTeamsColorVector(teams, teamColor); // TODO : S'assurer que ça fonctionne  et que le programme ne plante pas
    }
