...
void RaceResultGUI::displaySoccerResults()
{
    ...
    const int red_score = sw->getScore(RaceManager::get()->getTeamsInGame()[0]);
    const int blue_score = sw->getScore(RaceManager::get()->getTeamsInGame()[1]);
    ...
    drawTeamScorers(RaceManager::get()->getTeamsInGame()[0], current_x, current_y, height);
    drawTeamScorers(RaceManager::get()->getTeamsInGame()[1], current_x, current_y, height);
#endif
} // displaySoccerResults
...
{
#ifndef SERVER_ONLY
    int current_x = (team == RaceManager::get()->getTeamsInGame()[0]) ? x : x + UserConfigParams::m_width / 2;
    int current_y = y;
    core::rect<s32> pos(current_x, current_y, current_x, current_y);
    int prev_y = y;
