...
BattleTeamAI::BattleTeamAI(AbstractKart* kart)
    : ArenaAI(kart)
{
    ...
    Controller::setControllerName("BattleTeamAI");

}   // BattleTeamAI

//-----------------------------------------------------------------------------
BattleTeamAI::~BattleTeamAI()
{
#ifdef AI_DEBUG
 ...
 */
void BattleTeamAI::findClosestKart(bool consider_difficulty, bool find_sta)
{
        ...
        start_id < end; start_id++)
    {
        ...
        if (m_world->getKartTeams(kart->getWorldKartId()) == m_world->getKartTeams(m_kart->getWorldKartId()))
        {
            continue; // AIIMPORTANT
        ...
        if (dist_to_kart <= distance && kart->getTeam(start_id) != m_world->getKart(start_id)->getTeam(start_id))
        {
...
//-----------------------------------------------------------------------------
int BattleTeamAI::getCurrentNode() const
{
    return m_world->getSectorForKart(m_kart);
}   // getCurrentNode

//-----------------------------------------------------------------------------
bool BattleTeamAI::isWaiting() const
{
    return m_world->isStartPhase();
}   // isWaiting

//-----------------------------------------------------------------------------
float BattleTeamAI::getKartDistance(const AbstractKart* kart) const
{
...
//-----------------------------------------------------------------------------
bool BattleTeamAI::isKartOnRoad() const
{
