...
#include "items/small_soccer_ball.hpp"
...
float SmallSoccerBall::m_st_max_distance;   // maximum distance for a bowling ball to be attracted
float SmallSoccerBall::m_st_max_distance_squared;
float SmallSoccerBall::m_st_force_to_target;

// -----------------------------------------------------------------------------
SmallSoccerBall::SmallSoccerBall(Kart* kart)
    : Flyable(kart, PowerupManager::POWERUP_SMALL_SOCCER_BALL, 50.0f /* mass */)
{
    m_has_hit_kart = false;
    m_roll_sfx = SFXManager::get()->createSoundSource("shoot"); // TODO : NEED CHANGE
 ...
 */
SmallSoccerBall::~SmallSoccerBall()
{
 ...
 *  \param node XML Node
 *  \param bowling The Small Soccer ball mesh
 */
void SmallSoccerBall::init(const XMLNode& node, scene::IMesh* bowling)
{
    Flyable::init(node, bowling, PowerupManager::POWERUP_SMALL_SOCCER_BALL);
...
// ----------------------------------------------------------------------------
/** Updates the Small Soccer ball ineach frame. If this function returns true, the
 *  object will be removed by the projectile manager.
 *  \param dt Time step size.
 *  \returns True of this object should be removed.
 */
bool SmallSoccerBall::updateAndDelete(int ticks)
{
 ...
 */
bool SmallSoccerBall::hit(Kart* kart, PhysicalObject* obj)
{
...
// ----------------------------------------------------------------------------
void SmallSoccerBall::removeRollSfx()
{
    if (m_roll_sfx)
    {
 ...
 */
HitEffect* SmallSoccerBall::getHitEffect() const
{
...
// ----------------------------------------------------------------------------
void SmallSoccerBall::onFireFlyable()
{
