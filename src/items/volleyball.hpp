...
#ifndef HEADER_VOLLEYBALL_HPP
#define HEADER_VOLLEYBALL_HPP
namespace irr
{
...
/**
  * \ingroup items
  */
class Volleyball : public Flyable
{
private:
    static float m_st_max_distance;   // maximum distance for a volleyball ball to be attracted
    static float m_st_max_distance_squared;
    static float m_st_force_to_target;

    /** If a volleyball ball has hit something, this flag is set to indicate
...
public:
    Volleyball(Kart* kart);
    virtual ~Volleyball();
    static  void init(const XMLNode& node, scene::IMesh* volleyball);
    ...
    // ------------------------------------------------------------------------
    virtual void onFireFlyable() OVERRIDE; // bool mini
