...
void SoccerWorld::reset(bool restart)
{
    ...
    {
        if (getKartTeam(i) == RaceManager::get()->getTeamsInGame()[0])
            red_id.push_back(i);
        else
            blue_id.push_back(i);
    }
...
bool SoccerWorld::isRaceOver()
{
    ...
    {
        return (getScore(RaceManager::get()->getTeamsInGame()[1]) >= m_goal_target ||
            getScore(RaceManager::get()->getTeamsInGame()[0]) >= m_goal_target);
    }
...
bool SoccerWorld::getKartSoccerResult(unsigned int kart_id) const
{
    ...
    if ((red_win && team == RaceManager::get()->getTeamsInGame()[1]) ||
        (!red_win && team == RaceManager::get()->getTeamsInGame()[0]))
        return true;
    else
        return false;
...
void SoccerWorld::updateAIData()
{
    ...
    for (unsigned int i = 0; i < m_karts.size(); ++i)
    {
        ...
        if (getKartTeam(m_karts[i]->getWorldKartId()) == RaceManager::get()->getTeamsInGame()[1])
        {
...
{
    if (team == RaceManager::get()->getTeamsInGame()[0] && m_blue_kdm.size() > 1)
    {
    ...
    else if (team == RaceManager::get()->getTeamsInGame()[1] && m_red_kdm.size() > 1)
    {
...
void SoccerWorld::enterRaceOverState()
{
    ...
    if (UserConfigParams::m_arena_ai_stats)
    {
        ...
        if (getScore(RaceManager::get()->getTeamsInGame()[0]) >= m_goal_target)
