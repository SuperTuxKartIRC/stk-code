...
void World::init()
{
    ...
    if ((RaceManager::get()->getMinorMode() == RaceManager::MINOR_MODE_3_STRIKES                        ||
         ...
         RaceManager::get()->getMinorMode() == RaceManager::MINOR_MODE_TEAM_ARENA_BATTLE_POINTS_TEAM    || // TODO : NEED CHANGES ???
...
void World::initTeamsArrows(AbstractKart* k)
{
    ...
    std::string team1_path =
        file_manager->getAsset(FileManager::GUI_ICON, "red_arrow.png");
    std::string team2_path =
        file_manager->getAsset(FileManager::GUI_ICON, "blue_arrow.png");
    std::string team3_path =
        file_manager->getAsset(FileManager::GUI_ICON, "green_arrow.png");
    std::string team4_path =
    ...
    arrow_node = irr_driver->addBillboard(
        core::dimension2d<irr::f32>(0.3f, 0.3f),
        team == KART_TEAM_1 ? team1_path :
                KART_TEAM_2 ? team2_path :
                KART_TEAM_3 ? team3_path : team4_path,
        k->getNode());
...
//-----------------------------------------------------------------------------
void World::setAITeam()//AIIMPORTANT
{
    ...
    HandicapLevel handicap)
{
    ...
    {
        m_kart_teams_map[index] = RaceManager::get()->getKartInfo(index).getKartTeams();
...
KartTeams World::getKartTeams(unsigned int kart_id) const
{
    ...
    assert(n != m_kart_teams_map.end());//AIIMPORTANT
    return n->second;
}   // getKartTeams
...
void World::setAITeams()
{
    ...
    for (int i = 0; i < (int)RaceManager::get()->getNumLocalPlayers(); i++)
    {
        ...
        if (team == KART_TEAM_N)
        {
            RaceManager::get()->setKartTeams(i, KART_TEAM_2, KART_TEAM_COLOR_BLUE); // TODO : NEED MODIFICATIONS ??? KART_TEAM_COLOR_BLUE ???
            team = KART_TEAM_2;
            continue; //FIXME, this is illogical
        }
    }

    Log::debug("World", "Team1 AI: %d Team2 AI: %d Team3 AI: %d Team4 AI: %d", m_team1_ai, m_team2_ai, m_team3_ai, m_team4_ai); // TODO : NEED MODIFICATIONS ???

}   // setAITeams

//std::shared_ptr<GE::GERenderInfo> getGERenderInfo(KartTeamsColor teamColor)
//{
//    return teamColor == KART_TEAM_COLOR_RED ? std::make_shared < GE::GERenderInfo>(0.0f) :
