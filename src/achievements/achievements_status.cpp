...
void AchievementsStatus::setEnumToString()
{
    ...
    m_tr_enum_to_xml[(int)TR_FINISHED_POWERUP] = "race-finished-powerup";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_POWERUP] = "race-finished-no-powerup";
    m_tr_enum_to_xml[(int)TR_FINISHED_NITRO] = "race-finished-nitro";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_NITRO] = "race-finished-no-nitro";
    m_tr_enum_to_xml[(int)TR_FINISHED_BANANA] = "race-finished-banana";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_BANANA] = "race-finished-no-banana";
    ...
    m_tr_enum_to_xml[(int)TR_FINISHED_POWERUP + (int)TR_DATA_NUM] = "race-finished-powerup-all";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_POWERUP + (int)TR_DATA_NUM] = "race-finished-no-powerup-all";
    m_tr_enum_to_xml[(int)TR_FINISHED_NITRO + (int)TR_DATA_NUM] = "race-finished-nitro-all";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_NITRO + (int)TR_DATA_NUM] = "race-finished-no-nitro-all";
    m_tr_enum_to_xml[(int)TR_FINISHED_BANANA + (int)TR_DATA_NUM] = "race-finished-banana-all";
    m_tr_enum_to_xml[(int)TR_FINISHED_NO_BANANA + (int)TR_DATA_NUM] = "race-finished-no-banana-all";
        ...
        for (unsigned int i=0; i < xml_achievement_tracks.size(); i++)
        {
            ...
            for (unsigned int j=0 ; j < m_track_stats.size(); j++)
            {
                if (ident == m_track_stats[j].ident)
                {
                    ...
                    xml_achievement_tracks[i]->get("fin_powup", &m_track_stats[j].track_data[(int)TR_FINISHED_POWERUP]);
                    xml_achievement_tracks[i]->get("fin_nitro", &m_track_stats[j].track_data[(int)TR_FINISHED_NITRO]);
                    xml_achievement_tracks[i]->get("fin_banana", &m_track_stats[j].track_data[(int)TR_FINISHED_BANANA]);
            ...
            if (!track_found)
            {
                ...
                xml_achievement_tracks[i]->get("fin_powup", &new_track.track_data[(int)TR_FINISHED_POWERUP]);
                xml_achievement_tracks[i]->get("fin_nitro", &new_track.track_data[(int)TR_FINISHED_NITRO]);
                xml_achievement_tracks[i]->get("fin_banana",&new_track.track_data[(int)TR_FINISHED_BANANA]);
...
void AchievementsStatus::save(UTFWriter &out)
{
    ...
    for (unsigned int n = 0; n < m_track_stats.size(); n++)
    {
        ...
        out << " fin_powup=\"" << m_track_stats[n].track_data[(int)TR_FINISHED_POWERUP] << "\"";
        out << " fin_nitro=\"" << m_track_stats[n].track_data[(int)TR_FINISHED_NITRO] << "\"";
        out << " fin_banana=\"" << m_track_stats[n].track_data[(int)TR_FINISHED_BANANA] << "\"";
